...
--drop function [dbo].[tfnWareHouseListGet]


/****** Object:  UserDefinedFunction [dbo].[tfnWareHouseListGet]    Script Date: 02/26/2009 13:58:14 ******/
IF  NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[tfnWareHouseListGet]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
BEGIN	
	print 'Create function [dbo].[tfnWareHouseListGet] ' + convert(varchar, getdate())

	declare @create as varchar(2048)

	set @create = '

		Create function [dbo].[tfnWareHouseListGet] 
		(
...
print 'Update function [dbo].[tfnWareHouseListGet] ... ' + convert(varchar, getdate())
...
ALTER function [dbo].[tfnWareHouseListGet]
(
	@pCompanyId int
)  
RETURNS @Ret_Table Table
...
select * from [dbo].[tfnWareHouseListGet](3,7,8,24,1)
