/****** Object:  UserDefinedFunction [dbo].[sfnShippingOrderUnPaidTaxPrice]    Script Date: 02/26/2009 13:58:14 ******/
IF  NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sfnShippingOrderUnPaidTaxPrice]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
BEGIN
	print 'Create function [dbo].[sfnShippingOrderUnPaidTaxPrice] ' + convert(varchar, getdate())

	declare @create as varchar(2048)

	set @create = '

		Create function [dbo].[sfnShippingOrderUnPaidTaxPrice] 
		(
		)
		returns  bit
...
print 'Update function [dbo].[sfnShippingOrderUnPaidTaxPrice] ... ' + convert(varchar, getdate())
...
ALTER function [dbo].[sfnShippingOrderUnPaidTaxPrice]
(
...
Begin 
	...
	select @RtnValue = @pTaxPrice - isnull(@pChargedPrice, 0.0)
...
select [dbo].[sfnShippingOrderUnPaidTaxPrice]('0', 11)
select [dbo].[sfnShippingOrderUnPaidTaxPrice](2, 'AroundToday')

select [dbo].[sfnShippingOrderUnPaidTaxPrice](3, 'AroundToday')
