...
/****** Object:  Table [dbo].[ProviderLanguage]    Script Date: 01/15/2015 21:55:07 ******/
SET ANSI_NULLS ON
GO
...
Begin
	...
	alter table [ProviderLanguageCert] drop constraint [FK_ProviderLanguageCert_ProviderLanguage]
	...
	(   select fk.constraint_object_id from sys.foreign_key_columns as fk
		where fk.referenced_object_id = 
    		(select object_id from sys.tables where name = 'ProviderLanguage')
	)	
	
	declare @TblName as nvarchar(64) = 'ProviderLanguage'
	select distinct '
...
'
	...
	*/
	...
	print 'you need to manually exec DROP TABLE [dbo].[ProviderLanguage] ' + convert(varchar, getdate())
...
CREATE TABLE [dbo].[ProviderLanguage](
	...
	[Active] [bit] NOT NULL,			-- This is used to temporarily add provider language (set as Active false) so that we can add assoicated certificates before 
										-- the save at the end of the language defination process.
										
	[ProficiencyCodeId] [int] NOT NULL,
 ...
 CONSTRAINT [PK_ProviderLanguage] PRIMARY KEY CLUSTERED 
(
	[Id] ASC
...
--ALTER TABLE [dbo].[ProviderLanguage] ADD  CONSTRAINT [DF_ProviderLanguage_CreateDate]  DEFAULT (getdate()) FOR [CreateDate]
--ALTER TABLE [dbo].[ProviderLanguage] ADD  CONSTRAINT [DF_ProviderLanguage_LastUpdate]  DEFAULT (getdate()) FOR [LastUpdate]
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[ProviderLanguage]') AND type in (N'U'))
	...
	and NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[FK_ProviderLanguageCert_ProviderLanguage]') AND type in (N'F'))
begin
...
/****** Object:  Index [IX_ProviderLanguage_1]    Script Date: 01/16/2015 11:50:36 ******/
IF  EXISTS (SELECT * FROM sys.indexes WHERE object_id = OBJECT_ID(N'[dbo].[ProviderLanguage]') AND name = N'IX_ProviderLanguage_1')
DROP INDEX [IX_ProviderLanguage_1] ON [dbo].[ProviderLanguage]
GO




/****** Object:  Index [IX_ProviderLanguage_1]    Script Date: 2/2/2015 9:14:23 AM ******/
CREATE UNIQUE NONCLUSTERED INDEX [IX_ProviderLanguage_1] ON [dbo].[ProviderLanguage]
(
	[ProviderId] ASC,
	[LanguageId] ASC
