...
/****** Object:  Table [dbo].[RegionY]    Script Date: 01/15/2015 21:55:07 ******/
SET ANSI_NULLS ON
GO
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[RegionY]') AND type in (N'U'))
Begin
	/*
	
	DROP TABLE [dbo].[RegionY]
	...
	(   select fk.constraint_object_id from sys.foreign_key_columns as fk
		where fk.referenced_object_id = 
    		(select object_id from sys.tables where name = 'RegionY')
	)	
	
	
	declare @TblName as nvarchar(64) = 'RegionY'
	select distinct '
...
'
	...
	*/
	...
	print 'you need to manually exec DROP TABLE [dbo].[RegionY] ' + convert(varchar, getdate())
...
ALTER TABLE [dbo].[RegionY] ADD  CONSTRAINT [DF_RegionY_CreateDate]  DEFAULT (getdate()) FOR [CreateDate]
...
IF  EXISTS (SELECT * FROM sys.indexes WHERE object_id = OBJECT_ID(N'[dbo].[RegionY]') AND name = N'IX_RegionY_1')
DROP INDEX [IX_RegionY_1] ON [dbo].[RegionY]
GO
...
CREATE UNIQUE NONCLUSTERED INDEX [IX_RegionY_1] ON [dbo].[RegionY]
(
	[RegionId] ASC,
