...
/****** Object:  Table [dbo].[RequestFeedback]    Script Date: 01/15/2015 21:55:07 ******/
SET ANSI_NULLS ON
GO
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[RequestFeedback]') AND type in (N'U'))
Begin
	/*
	
	DROP TABLE [dbo].[RequestFeedback]
	...
	(   select fk.constraint_object_id from sys.foreign_key_columns as fk
		where fk.referenced_object_id = 
    		(select object_id from sys.tables where name = 'RequestFeedback')
	)	
	

	declare @TblName as nvarchar(64) = 'RequestFeedback'
	select distinct '
...
'
	...
	*/
	...
	print 'you need to manually exec DROP TABLE [dbo].[RequestFeedback] ' + convert(varchar, getdate())
...
ALTER TABLE [dbo].[RequestFeedback] ADD  CONSTRAINT [DF_RequestFeedback_CreateDate]  DEFAULT (getutcdate()) FOR [CreateDate]
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Request]') AND type in (N'U'))
	and EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[RequestFeedback]') AND type in (N'U'))
	and NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[FK_RequestFeedback_Request]') AND type in (N'F'))
begin
...
/****** Object:  Index [IX_RequestFeedback_1]    Script Date: 01/16/2015 11:50:36 ******/
IF  EXISTS (SELECT * FROM sys.indexes WHERE object_id = OBJECT_ID(N'[dbo].[RequestFeedback]') AND name = N'IX_RequestFeedback_1')
DROP INDEX [IX_RequestFeedback_1] ON [dbo].[RequestFeedback]
GO




/****** Object:  Index [IX_RequestFeedback_1]    Script Date: 2/2/2015 9:14:23 AM ******/
CREATE NONCLUSTERED INDEX [IX_RequestFeedback_1] ON [dbo].[RequestFeedback]
(
	[RequestId] ASC
