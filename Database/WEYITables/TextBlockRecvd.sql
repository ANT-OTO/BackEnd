...
/****** Object:  Table [dbo].[TextBlockRecvd]    Script Date: 01/15/2015 21:55:07 ******/
SET ANSI_NULLS ON
GO
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[TextBlockRecvd]') AND type in (N'U'))
Begin
	/*


	DROP TABLE [dbo].[TextBlockRecvd]
	...
	(   select fk.constraint_object_id from sys.foreign_key_columns as fk
		where fk.referenced_object_id = 
    		(select object_id from sys.tables where name = 'TextBlockRecvd')
	)	
	
	

	declare @TblName as nvarchar(64) = 'TextBlockRecvd'
	select distinct '
...
'
	...
	*/
	...
	print 'you need to manually exec DROP TABLE [dbo].[TextBlockRecvd] ' + convert(varchar, getdate())
	
End
else
begin

CREATE TABLE [dbo].[TextBlockRecvd](
	[Id] [int] IDENTITY(1,1) NOT NULL,

	[TextSessionId] [int] NOT NULL,	

	[FromCaller] bit NOT NULL,			

	[TextBlockId] [int] NOT NULL,	

	[TextBlockTextId] [int] NULL,	
	[TextBlockImageActionId] [int] NULL,	
	[TextBlockAudioActionId] [int] NULL,	

	[Version] [timestamp] NOT NULL,
	[CreateDate] [datetime] NOT NULL,
	[LastUpdate] [datetime] NOT NULL,
 CONSTRAINT [PK_TextBlockRecvd] PRIMARY KEY CLUSTERED 
(
	[Id] ASC
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[TextBlockImageAction]') AND type in (N'U'))
	and EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[TextBlockRecvd]') AND type in (N'U'))
	and NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[FK_TextBlockRecvd_TextBlockImageAction]') AND type in (N'F'))
begin
	ALTER TABLE [dbo].[TextBlockRecvd]  WITH CHECK ADD  CONSTRAINT [FK_TextBlockRecvd_TextBlockImageAction] FOREIGN KEY([TextBlockImageActionId])
	REFERENCES [dbo].[TextBlockImageAction] ([Id])

	ALTER TABLE [dbo].[TextBlockRecvd] CHECK CONSTRAINT [FK_TextBlockRecvd_TextBlockImageAction]
end
...
/****** Object:  Index [IX_TextBlockRecvd_1]    Script Date: 01/16/2015 11:50:36 ******/
IF  EXISTS (SELECT * FROM sys.indexes WHERE object_id = OBJECT_ID(N'[dbo].[TextBlockRecvd]') AND name = N'IX_TextBlockRecvd_1')
DROP INDEX [IX_TextBlockRecvd_1] ON [dbo].[TextBlockRecvd]
GO




/****** Object:  Index [IX_TextBlockRecvd_1]    Script Date: 2/2/2015 9:14:23 AM ******/
CREATE NONCLUSTERED INDEX [IX_TextBlockRecvd_1] ON [dbo].[TextBlockRecvd]
(
	[TextSessionId] ASC,
	[FromCaller] ASC,
	[TextBlockId] ASC
