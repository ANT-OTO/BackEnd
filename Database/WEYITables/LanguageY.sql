...
/****** Object:  Table [dbo].[LanguageY]    Script Date: 01/15/2015 21:55:07 ******/
SET ANSI_NULLS ON
GO
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[LanguageY]') AND type in (N'U'))
Begin
	/*
	
	DROP TABLE [dbo].[LanguageY]
	...
	(   select fk.constraint_object_id from sys.foreign_key_columns as fk
		where fk.referenced_object_id = 
    		(select object_id from sys.tables where name = 'LanguageY')
	)	
	

	declare @TblName as nvarchar(64) = 'LanguageY'
	select distinct '
...
'
	...
	*/
	...
	print 'you need to manually exec DROP TABLE [dbo].[LanguageY] ' + convert(varchar, getdate())
	
End
else
begin

CREATE TABLE [dbo].[LanguageY](
	[Id] [int] IDENTITY(1,1) NOT NULL,
	[LanguageId] [int] NOT NULL,
	[SystemLanguageId] [int] NOT NULL,
	[Name] [nvarchar](256) NOT NULL,
	[CreateDate] [datetime] NOT NULL,
 CONSTRAINT [PK_LanguageY] PRIMARY KEY CLUSTERED 
(
	[Id] ASC
...
ALTER TABLE [dbo].[LanguageY] ADD  CONSTRAINT [DF_LanguageY_CreateDate]  DEFAULT (getdate()) FOR [CreateDate]
...
IF  EXISTS (SELECT * FROM sys.indexes WHERE object_id = OBJECT_ID(N'[dbo].[LanguageY]') AND name = N'IX_LanguageY_1')
DROP INDEX [IX_LanguageY_1] ON [dbo].[LanguageY]
GO
...
CREATE UNIQUE NONCLUSTERED INDEX [IX_LanguageY_1] ON [dbo].[LanguageY]
(
	[LanguageId] ASC,
