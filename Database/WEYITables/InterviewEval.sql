...
/****** Object:  Table [dbo].[InterviewEval]    Script Date: 01/15/2015 21:55:07 ******/
SET ANSI_NULLS ON
GO
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[InterviewEval]') AND type in (N'U'))
Begin
	/*
	
	DROP TABLE [dbo].[InterviewEval]
	...
	(   select fk.constraint_object_id from sys.foreign_key_columns as fk
		where fk.referenced_object_id = 
    		(select object_id from sys.tables where name = 'InterviewEval')
	)	
	


	declare @TblName as nvarchar(64) = 'InterviewEval'
	select distinct '
...
'
	...
	*/
	...
	print 'you need to manually exec DROP TABLE [dbo].[InterviewEval] ' + convert(varchar, getdate())
	
End
else
begin

CREATE TABLE [dbo].[InterviewEval](
 ...
 CONSTRAINT [PK_InterviewEval] PRIMARY KEY CLUSTERED 
(
	[Id] ASC
...
ALTER TABLE [dbo].[InterviewEval] ADD  CONSTRAINT [DF_InterviewEval_CreateDate]  DEFAULT (getdate()) FOR [CreateDate]
...
/****** Object:  Index [IX_InterviewEval_1]    Script Date: 01/16/2015 11:50:36 ******/
IF  EXISTS (SELECT * FROM sys.indexes WHERE object_id = OBJECT_ID(N'[dbo].[InterviewEval]') AND name = N'IX_InterviewEval_1')
DROP INDEX [IX_InterviewEval_1] ON [dbo].[InterviewEval]
GO




/****** Object:  Index [IX_InterviewEval_1]    Script Date: 2/2/2015 9:14:23 AM ******/
CREATE NONCLUSTERED INDEX [IX_InterviewEval_1] ON [dbo].[InterviewEval]
(
	[InterviewId] ASC,
	[EvalQuestionId] ASC
