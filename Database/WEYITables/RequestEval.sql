...
/****** Object:  Table [dbo].[RequestEval]    Script Date: 01/15/2015 21:55:07 ******/
SET ANSI_NULLS ON
GO
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[RequestEval]') AND type in (N'U'))
Begin
	/*
	
	DROP TABLE [dbo].[RequestEval]
	...
	(   select fk.constraint_object_id from sys.foreign_key_columns as fk
		where fk.referenced_object_id = 
    		(select object_id from sys.tables where name = 'RequestEval')
	)	
	
	
	declare @TblName as nvarchar(64) = 'RequestEval'
	select distinct '
...
'
	...
	*/
	...
	print 'you need to manually exec DROP TABLE [dbo].[RequestEval] ' + convert(varchar, getdate())
	
End
else
begin

CREATE TABLE [dbo].[RequestEval](
 ...
 CONSTRAINT [PK_RequestEval] PRIMARY KEY CLUSTERED 
(
	[Id] ASC
...
/****** Object:  Index [IX_RequestEval_1]    Script Date: 01/16/2015 11:50:36 ******/
IF  EXISTS (SELECT * FROM sys.indexes WHERE object_id = OBJECT_ID(N'[dbo].[RequestEval]') AND name = N'IX_RequestEval_1')
DROP INDEX [IX_RequestEval_1] ON [dbo].[RequestEval]
GO




/****** Object:  Index [IX_RequestEval_1]    Script Date: 2/2/2015 9:14:23 AM ******/
CREATE NONCLUSTERED INDEX [IX_RequestEval_1] ON [dbo].[RequestEval]
(
	[RequestId] ASC,
	[EvalQuestionId] ASC
