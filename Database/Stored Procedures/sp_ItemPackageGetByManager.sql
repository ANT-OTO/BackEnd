IF  NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_ItemPackageGetByManager]') AND type in (N'P', N'PC'))
BEGIN
	print 'Create procedure [dbo].[sp_ItemPackageGetByManager] ' + convert(varchar, getdate())

	declare @create as varchar(2048)

	set @create = '

		Create procedure [dbo].[sp_ItemPackageGetByManager] 
		AS 
		BEGIN 
			PRINT ''To begin''
		END
...
print 'Update [sp_ItemPackageGetByManager] ... ' + convert(varchar, getdate())
...
Alter procedure [dbo].[sp_ItemPackageGetByManager] 
...
SET NOCOUNT ON
	...
	select	a.Id as [SourceId], 'ItemSubmitPackage' as [SourceTable],
			...
			a.UserId as [UserId], d.Email as [UserEmail], a.CompanyId as [CompanyId],
