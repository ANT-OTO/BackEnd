IF  NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_Customer_OrderSearch]') AND type in (N'P', N'PC'))
BEGIN
	print 'Create procedure [dbo].[sp_Customer_OrderSearch] ' + convert(varchar, getdate())

	declare @create as varchar(2048)

	set @create = '

		Create procedure [dbo].[sp_Customer_OrderSearch] 
		AS 
		BEGIN 
			PRINT ''To begin''
		END
...
print 'Update [sp_Customer_OrderSearch] ... ' + convert(varchar, getdate())
...
Alter procedure [dbo].[sp_Customer_OrderSearch] 
	@pProductName nvarchar(256),
    @pCustomerName nvarchar(256),
	@pCustomerThirdPartyId nvarchar(256),
...
SET NOCOUNT ON
	...
	select @pCustomerName = isnull(@pCustomerName, '')
    select @pCustomerName = rtrim(ltrim(@pCustomerName))

	select @pCustomerThirdPartyId = isnull(@pCustomerThirdPartyId, '')
    select @pCustomerThirdPartyId = rtrim(ltrim(@pCustomerThirdPartyId))
	...
	declare @pByCustomerTypeCodeId bit = 0
	if(@pCustomerTypeCodeId is null or @pCustomerTypeCodeId <= 0)
	begin
		select @pByCustomerTypeCodeId = 0
	end
	else
	begin
		select @pByCustomerTypeCodeId = 1
	end
	...
	and (@pByCustomerTypeCodeId = 0 or (@pByCustomerTypeCodeId = 1 and z.CustomerTypeCodeId = @pCustomerTypeCodeId))
	...
	and (@pCustomerName = '' or (b.NickName like '%' + @pCustomerName + '%')
	or (b.FirstName like '%' + @pCustomerName + '%')
	or (b.LastName like '%' + @pCustomerName + '%')
	or (b.Email like '%' + @pCustomerName + '%')
	or (z.Id is not null and z.NickName like '%' + @pCustomerName + '%')
	)
	and (@pCustomerThirdPartyId = '' or (z.Id is not null and z.UnifiedId = @pCustomerThirdPartyId))
