IF  NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_Item_GetFromItemId]') AND type in (N'P', N'PC'))
BEGIN
	print 'Create procedure [dbo].[sp_Item_GetFromItemId] ' + convert(varchar, getdate())

	declare @create as varchar(2048)

	set @create = '

		Create procedure [dbo].[sp_Item_GetFromItemId] 
		AS 
		BEGIN 
			PRINT ''To begin''
		END
...
print 'Update [sp_Item_GetFromItemId] ... ' + convert(varchar, getdate())
...
Alter procedure [dbo].[sp_Item_GetFromItemId] 
	@pItemId int,
	@pCompanyId int,
