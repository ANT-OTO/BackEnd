...
Begin
	/*
	alter table [ItemSizeInfoY] drop constraint [FK_ItemSizeInfoY_ItemSizeInfo]
	DROP TABLE [dbo].[ItemSizeInfo]
	...
	(   select fk.constraint_object_id from sys.foreign_key_columns as fk
		where fk.referenced_object_id = 
    		(select object_id from sys.tables where name = 'ItemSizeInfo')
	)
	
	
	declare @TblName as nvarchar(64) = 'ItemSizeInfo'
	select distinct '
...
'
	...
	*/
	print 'you need to manually exec DROP TABLE [dbo].[ItemSizeInfo] ' + convert(varchar, getdate())
...
CREATE TABLE [dbo].[ItemSizeInfo](
 ...
 CONSTRAINT [PK_ItemSizeInfo] PRIMARY KEY CLUSTERED 
(
	[Id] ASC
...
ALTER TABLE [dbo].[ItemSizeInfo] ADD  CONSTRAINT [DF_ItemSizeInfo_CreateDate]  DEFAULT (getdate()) FOR [CreateDate]
ALTER TABLE [dbo].[ItemSizeInfo] ADD  CONSTRAINT [DF_ItemSizeInfo_LastUpdate]  DEFAULT (getdate()) FOR [LastUpdate]
...
/****** Object:  Index [IX_ItemSizeInfo_1]    Script Date: 01/16/2015 11:50:36 ******/
IF  EXISTS (SELECT * FROM sys.indexes WHERE object_id = OBJECT_ID(N'[dbo].[ItemSizeInfo]') AND name = N'IX_ItemSizeInfo_1')
DROP INDEX [IX_ItemSizeInfo_1] ON [dbo].[ItemSizeInfo]
GO
