...
/****** Object:  Table [dbo].[CodeListY]    Script Date: 01/15/2015 21:55:07 ******/
SET ANSI_NULLS ON
GO
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[CodeListY]') AND type in (N'U'))
Begin
	/*
	
	DROP TABLE [dbo].[CodeListY]
	...
	(   select fk.constraint_object_id from sys.foreign_key_columns as fk
		where fk.referenced_object_id = 
    		(select object_id from sys.tables where name = 'CodeListY')
	)	
	
	
	declare @TblName as nvarchar(64) = 'CodeListY'
	select distinct '
...
'
	...
	*/
	...
	print 'you need to manually exec DROP TABLE [dbo].[CodeListY] ' + convert(varchar, getdate())
	
End
else
begin

CREATE TABLE [dbo].[CodeListY](
 ...
 CONSTRAINT [PK_CodeListY] PRIMARY KEY CLUSTERED 
(
	[Id] ASC
...
ALTER TABLE [dbo].[CodeListY] ADD  CONSTRAINT [DF_CodeListY_CreateDate]  DEFAULT (getdate()) FOR [CreateDate]
...
/****** Object:  Index [IX_CodeListY_1]    Script Date: 01/16/2015 11:50:36 ******/
IF  EXISTS (SELECT * FROM sys.indexes WHERE object_id = OBJECT_ID(N'[dbo].[CodeListY]') AND name = N'IX_CodeListY_1')
DROP INDEX [IX_CodeListY_1] ON [dbo].[CodeListY]
GO




/****** Object:  Index [IX_CodeListY_1]    Script Date: 2/2/2015 9:14:23 AM ******/
CREATE UNIQUE NONCLUSTERED INDEX [IX_CodeListY_1] ON [dbo].[CodeListY]
(
	[CodeListId] ASC,
